...
def load_features_and_labels(radiomics_path, labels_df):
    ...
    if all(c in radiomics_df.columns for c in [SPLIT_COL, FOLD_COL, LABEL_COL]):
        merged_df = radiomics_df.copy().reset_index(drop=True)
    else:
        ...
        if len(merged_df) != len(radiomics_df) or len(merged_df) != len(labels_df):
            raise ValueError(f"Clinical features and labels are not a complete one-to-one match: features={len(radiomics_df)}, labels={len(labels_df)}, merged={len(merged_df)}")

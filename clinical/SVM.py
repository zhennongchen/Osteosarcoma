...
NON_FEATURE_COLS = ["Patient_set", "Patient_index", "Image_filepath", "Mask_filepath", "Prognosis_label", "Pathologic_label", "split", "fold"]
ID_COLS = ["Patient_set", "Patient_index", "Image_filepath", "Mask_filepath"]
RFECV_MAX_FEATURES = 24
LASSO_MAX_FEATURES = 24
SPLIT_COL = "split"
FOLD_COL = "fold"
...
PCC_RADIOMICS_PATH = "/host/d/projects/Habitats/radiomics/clinical_variables/clinical_variables_normalized.xlsx"
RADIOMICS_OUT_DIR = "/host/d/projects/Habitats/radiomics/clinical_variables"
SELECT_OUT_DIR = "/host/d/projects/Habitats/radiomics/clinical_variables/select"
...
SELECTOR_ARG = "svm_feature_selector"
DEFAULT_SELECTOR = "none"
SELECTED_PREFIX = "clinical_variables"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run clinical-variable SVM experiments.")
    ...
    parser.add_argument("--svm_feature_selector", choices=["none", "lasso", "rfecv"], default=DEFAULT_SELECTOR)
    parser.add_argument("--top_k", type=parse_top_k, default=None, help="Clinical selectors use None by default; lasso keeps all non-zero coefficients.")
...
def load_features_and_labels(radiomics_path, labels_df):
    ...
    if missing_radiomics_cols:
        raise ValueError(f"Missing columns in clinical feature table: {missing_radiomics_cols}")

    feature_cols = [c for c in radiomics_df.columns if c not in NON_FEATURE_COLS]
    if not feature_cols:
        raise ValueError("No clinical feature columns found after removing non-feature columns.")

    # Always use the current split file for split/fold/label.
    # The clinical feature table may contain stale split/fold columns from a previous split.
    label_cols = ["Patient_set", "Patient_index", SPLIT_COL, FOLD_COL, LABEL_COL]
    merge_left = radiomics_df.drop(
        columns=[c for c in [SPLIT_COL, FOLD_COL, LABEL_COL] if c in radiomics_df.columns],
        errors="ignore",
    )
    merged_df = merge_left.merge(
    ...
    if len(merged_df) != len(radiomics_df) or len(merged_df) != len(labels_df):
        raise ValueError(
            f"Clinical features and labels are not a complete one-to-one match: "
...
def get_selected_feature_path(task, random_state, feature_selector, top_k):
    suffix = f"{task}_random{random_state}_{feature_selector}_{top_k_label(top_k)}"
...
def save_selected_features(radiomics_df, selected_features, task, random_state, feature_selector, top_k):
    ...
    print("Saved selected clinical feature table:", selected_out_path)
...
def write_skip_file(out_dir, args, feature_selector, reason):
    ...
    skip_info = {"classifier": CLASSIFIER_NAME, "task": args.task, "label_col": LABEL_COL, "random_state": args.random_state, "feature_selector": feature_selector, "top_k": args.top_k, "status": "skipped", "reason": reason}

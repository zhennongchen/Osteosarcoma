...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["SVM"],
        default=None,
        help="Classifier branch to run. If omitted, only patient split is created.",
    )
    parser.add_argument(
        "--svm_feature_selector",
        choices=["rfe", "sfs", "rfecv"],
        default="rfe",
        help="SVM-specific supervised feature-selection method.",
    )
    parser.add_argument(
        "--top_k",
        type=int,
...
def make_linear_svm_pipeline(random_state, C=1.0, tol=1e-3):
    return Pipeline(
        [
...
def select_svm_features(feature_selector, top_k, feature_cols, X, y, random_state):
    cv = StratifiedKFold(
        n_splits=N_SPLITS,
        shuffle=True,
        random_state=random_state,
    )
    svm_pipe = make_linear_svm_pipeline(random_state=random_state)
    ...
    if feature_selector == "rfe":
        ...
        raise ValueError(f"Unsupported SVM feature selector: {feature_selector}")
...
def get_svm_selected_feature_path(random_state, feature_selector, top_k):
    ...
    return os.path.join(
        ...
        f"radiomics_measurements_SVM_{suffix}_selected.xlsx",
    )
...
def save_selected_svm_features(
    radiomics_df,
    selected_features,
    random_state,
    feature_selector,
    ...
    selected_out_path = get_svm_selected_feature_path(
        random_state=random_state,
...
def load_or_select_svm_features(
    radiomics_df,
    feature_selector,
    top_k,
    feature_cols,
    ...
    selected_path = get_svm_selected_feature_path(
        random_state=random_state,
        feature_selector=feature_selector,
        top_k=top_k,
    )
    ...
    _, selected_features = select_svm_features(
        feature_selector=feature_selector,
        top_k=top_k,
    ...
    selected_path = save_selected_svm_features(
...
def run_svm_experiment(args, labels_df):
    ...
    for fold_id in range(N_SPLITS):
        ...
        svm_fixed = make_linear_svm_pipeline(
            random_state=args.random_state,
...
def main():
    ...
    if args.classifier == "SVM":
        run_svm_experiment(args, labels_df)
    else:

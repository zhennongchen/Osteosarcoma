...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run whole-image radiomics A-line XGBoost experiments."
    )
    parser.add_argument(
        "--random_state",
        type=int,
    ...
    parser.add_argument(
        "--xgb_feature_selector",
        choices=["rfe", "sfs", "rfecv"],
        default="rfe",
        help="XGBoost-specific supervised feature-selection method.",
    )
    parser.add_argument(
        "--top_k",
        type=int,
...
def make_xgb_classifier(random_state, scale_pos_weight, **params):
...
def get_xgb_selected_feature_path(random_state, feature_selector, top_k):
    ...
    return os.path.join(
        ...
        f"radiomics_measurements_XGBoost_{suffix}_selected.xlsx",
    )


def select_xgb_features(feature_selector, top_k, feature_cols, X, y, random_state):
    cv = StratifiedKFold(
        n_splits=N_SPLITS,
        shuffle=True,
        random_state=random_state,
    )
    scale_pos_weight = scale_pos_weight_from_y(y)
    ...
    if feature_selector == "rfe":
        ...
        raise ValueError(f"Unsupported XGBoost feature selector: {feature_selector}")
...
def save_selected_xgb_features(radiomics_df, selected_features, random_state, feature_selector, top_k):
    os.makedirs(WHOLE_IMAGE_RADIOMICS_OUT_DIR, exist_ok=True)
    selected_out_path = get_xgb_selected_feature_path(
        random_state=random_state,
...
def load_or_select_xgb_features(
    radiomics_df,
    feature_selector,
    top_k,
    feature_cols,
    ...
    selected_path = get_xgb_selected_feature_path(
        random_state=random_state,
        feature_selector=feature_selector,
        top_k=top_k,
    )
    ...
    _, selected_features = select_xgb_features(
        feature_selector=feature_selector,
        top_k=top_k,
    ...
    selected_path = save_selected_xgb_features(
...
def get_xgb_experiment_name(random_state, feature_selector, top_k):
...
def write_skip_file(out_dir, args, reason):
    ...
    skip_info = {
        ...
        "feature_selector": args.xgb_feature_selector,
        "top_k": None if args.xgb_feature_selector == "rfecv" else args.top_k,
        "status": "skipped",
        "reason": reason,
    }
...
def run_xgb_experiment(args, labels_df):
    radiomics_df, merged_df, feature_cols, X, y, folds = load_features_and_labels(
        PCC_RADIOMICS_PATH,
        labels_df,
    )
    experiment_name = get_xgb_experiment_name(
        random_state=args.random_state,
        feature_selector=args.xgb_feature_selector,
        top_k=args.top_k,
    )
    out_dir = os.path.join(WHOLE_IMAGE_MODEL_OUT_DIR, "XGBoost", experiment_name)

    try:
        selected_path, selected_features = load_or_select_xgb_features(
            radiomics_df=radiomics_df,
            feature_selector=args.xgb_feature_selector,
            top_k=args.top_k,
            feature_cols=feature_cols,
    ...
    roc_path = os.path.join(out_dir, "ROC_curve_5foldCV_XGBoost.pdf")
    plot_roc_curve(
        y_true,
        y_prob,
...
def main():
    ...
    if args.classifier == "XGBoost":
        run_xgb_experiment(args, labels_df)
    else:

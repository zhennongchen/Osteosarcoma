...
for RANDOM_STATE in ${RANDOM_STATE_LIST}; do
  for selector in rfe; do
    for top_k in ${TOP_K_LIST}; do
      echo "========== SVM | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=${selector} | top_k=${top_k} =========="
  ...
  echo "========== SVM | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=rfecv =========="
  python3 "${PYTHON_SCRIPT}" \
    --classifier SVM \
    --task "${TASK}" \
    --pcc_radiomics_path "${PCC_RADIOMICS_PATH}" \
  ...
  for top_k in ${LASSO_TOP_K_LIST}; do
    echo "========== SVM | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=lasso | top_k=${top_k} =========="
    python3 "${PYTHON_SCRIPT}" \
      --classifier SVM \
      --task "${TASK}" \
      --pcc_radiomics_path "${PCC_RADIOMICS_PATH}" \

...
for RANDOM_STATE in ${RANDOM_STATE_LIST}; do
  for selector in lasso; do
    for top_k in ${TOP_K_LIST}; do
      echo "========== KNN | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=${selector} | top_k=${top_k} =========="
  ...
  for top_k in ${LASSO_TOP_K_LIST}; do
    echo "========== KNN | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=lasso | top_k=${top_k} =========="
    python3 "${PYTHON_SCRIPT}" \
      --classifier KNN \
      --task "${TASK}" \
      --gridsearch_range "${GRIDSEARCH_RANGE}" \

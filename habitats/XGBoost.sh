...
for RANDOM_STATE in ${RANDOM_STATE_LIST}; do
  ...
  echo "========== XGBoost | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=rfecv =========="
  python3 "${PYTHON_SCRIPT}" \
    --classifier XGBoost \
    --task "${TASK}" \
    --gridsearch_range "${GRIDSEARCH_RANGE}" \
  ...
  for top_k in ${LASSO_TOP_K_LIST}; do
    echo "========== XGBoost | task=${TASK} | habitat_mode=${HABITAT_MODE} | random_state=${RANDOM_STATE} | selector=lasso | top_k=${top_k} =========="
    python3 "${PYTHON_SCRIPT}" \
      --classifier XGBoost \
      --task "${TASK}" \
      --gridsearch_range "${GRIDSEARCH_RANGE}" \

...
def parse_args():
    parser = argparse.ArgumentParser(description='Train 2D / 2.5D CNN for Osteosarcoma MRI classification.')
    ...
    parser.add_argument('--model_depth', type=int, default=50, choices=[18, 34, 50], help='Depth of the ResNet model.')
    parser.add_argument('--efficientnet_version', type=str, default='b0', choices=['b0', 'b1', 'b2', 'b3', 'v2_s'])
    parser.add_argument('--init_channel', type=int, default=8, help='Initial channel count for shallow_cnn.')
    parser.add_argument('--fine_tune_stage', type=str, default='all', choices=['all', 'fc', '1', '2'])
    parser.add_argument('--only_tumor_pixels', type=str, default='roi', choices=['roi', 'seg'], help='Kept for backward compatibility; generator now always returns [full,bbox,tumor].')
...
def main():
    ...
    print('============================================================')
    print('2D / 2.5D CNN training settings')
    print('label:', label)
    ...
    x_file_list_train = [
        os.path.join(data_root, patient_set_list_train[i], patient_index_list_train[i], 'img_slices.nii.gz')
        for i in range(len(patient_index_list_train))
    ]
    y_list_train = [int(label_list_train[i]) for i in range(len(label_list_train))]

    x_file_list_val = [
        os.path.join(data_root, patient_set_list_val[i], patient_index_list_val[i], 'img_slices.nii.gz')
    ...
    train_generator = Generator.Dataset_2D(
    ...
    val_generator = Generator.Dataset_2D(
    ...
    # Anti-overfitting settings are intentionally defined here, not in train.sh.
    train_weight_decay = 5e-4
    label_smoothing = 0.05
    use_class_weight = True
    dropout_p = 0.3

    # If there is no project checkpoint, start from ImageNet pretrained weights.
    # If there is a checkpoint, build the same architecture and load the full saved state.
    use_imagenet = trained_model_path is None
    ...
    model = model_module.configure_fine_tuning(model, fine_tune_stage=fine_tune_stage)

    print('dropout_p:', dropout_p)
    print('label_smoothing:', label_smoothing)
    print('use_class_weight:', use_class_weight)
    print('train_weight_decay:', train_weight_decay)

...
OUT_DIR = TRIAL_DIR / 'gradcam' / f'epoch{CHECKPOINT_EPOCH}_layer4_predclass'
...
def run_one_case(model, gradcam, ds, subset_name, case_pos, global_idx):
    ...
    for s, (name, cam) in enumerate(zip(slice_names, cams)):
        ...
        titles = ['full image', 'bbox mask', 'tumor mask', f'Grad-CAM {name}']
        for j in range(4):
            ax = axes[s, j]
            if j == 3:
                ax.imshow(panels[j])
        ...
        records.append({
            ...
            'target_class_for_cam': pred_class,
            'slice': name,

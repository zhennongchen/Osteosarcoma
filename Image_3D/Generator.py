...
class Dataset_3D(Dataset):
    ...
    def load_file(self, filename, bbox_filename = None):
        ...
        # patient_set = os.path.basename(os.path.dirname(os.path.dirname(filename)))
        # print('now load patient set: ', patient_set, ' and patient index: ', patient_id)
        ...
        # cropped_nii = nb.Nifti1Image(ii, affine)
        # nb.save(cropped_nii, os.path.join(self.data_root, 'set_' + patient_set + '_patient_' + patient_id + '_cropped.nii.gz'))

        # preprocess the image
        ii = Data_processing.crop_or_pad(ii, [self.image_size[0], self.image_size[1], self.image_size[2]], value= np.min(ii))
        ...
        # print('cropped/padded image shape is: ', ii.shape)
        # pad_nii = Data_processing.normalize_image(ii, normalize_factor = self.normalize_factor, image_max = ii_max, image_min = ii_min, invert = True)
        # pad_nii = nb.Nifti1Image(ii, affine)
        # nb.save(pad_nii, os.path.join(self.data_root, 'set_' + patient_set + '_patient_' + patient_id + '_cropped_padded.nii.gz'))
        return ii
    ...
    def __getitem__(self, index):
        ...
        if self.augment == True:
            if random.uniform(0,1) < self.augment_frequency:
                img_numpy, z_rotate_degree = random_rotate(img_numpy, order = 1)
                img_numpy, translate, y_translate = random_translate(img_numpy)
        

        input_data = torch.from_numpy(img_numpy).unsqueeze(0).float()
